...
if __name__=='__main__':
    ...
    parser.add_argument('--thresh_list', nargs='+')
    ...
    for dataset in datasets:
        ...
        for idx, thresh in enumerate(thresh_list):
            path = f'outputs/{method2method[method]}/{method}/{dataset}_{n_rag}_{num_gen}_{thresh}_calibration_result.json'
        ...
        data = {
            'thresh': [ int(item) / 100. for item in thresh_list],
        ...
        for metric, color in color_dict.items():
            plt.plot(df['thresh'], df[metric], marker='^', color=color, label=r'Certified Conformal Risk $\alpha_{\text{rag}}$', markersize=20)

        simulation_points = []
        for idx_,thresh in enumerate(thresh_list):
            path = f'outputs/{method2method[method]}/{method}/{dataset}_{n_rag}_{num_gen}_{thresh}_calibration_result.json'
            result = json.load(open(path, 'r', encoding='utf-8'))
            result = list(result.values())[0]
            result = np.array(result)
            ...
            for x in simulation_points_temp:
                if x < min_risk:
                    min_risk = x
            plt.scatter(x=[int(thresh) / 100.0] * len(simulation_points_temp), y=simulation_points_temp, color='gray', alpha=0.7, s=100)
        ...
        plt.tight_layout()
        print('save figure at {}'.format({f'./figs/{dataset}_{method2name[method]}_multi_thresh_conformal_bound_simulation.jpg'}))
        plt.savefig(f'./figs/{dataset}_{method2name[method]}_multi_thresh_conformal_bound_simulation.jpg', dpi=800)