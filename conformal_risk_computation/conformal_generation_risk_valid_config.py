...
import numpy as np
import argparse
from utils import save_json_to_file, get_method2name, get_method2method, get_num_points, empirical_risk2alpha_func, compute_p_value, FWER
...
if __name__=='__main__':
    ...
    parser.add_argument('--alpha_desired', type=float, default=0.6)
    ...
    alpha_desired = args.alpha_desired
    delta = 0.1
    ...
    for dataset in datasets:
        ...
        for n_rag in n_rag_list:
            for num_gen in lambda_g_list:
                num_gen = int(num_gen)
                path = f'outputs/{method2method[method]}/{method}/{dataset}_{n_rag}_{max_num_gen}_0_calibration_result.json'
                ...
                p_ = compute_p_value(alpha_desired, np.mean(results), len(results))
                p_values[(n_rag, num_gen)] = p_

        valid_or_not = FWER(p_values, delta)

        for n_rag in n_rag_list:
            for num_gen in lambda_g_list:
                num_gen = int(num_gen)
                if (not valid_or_not[(n_rag, num_gen)]):
                    continue

                ax.scatter3D([n_rag], [num_gen], 0.01, c='green', alpha=0.9, s=200)
        ...
        lambda_g_list = [int(item) for item in lambda_g_list]

        N_rag_grid, lambda_g_grid = np.meshgrid(n_rag_list, lambda_g_list)
        risk_values = np.ones_like(N_rag_grid, dtype=float) * alpha_desired



        surf = ax.plot_surface(N_rag_grid, lambda_g_grid, risk_values, color='red', edgecolor='none', alpha=0.2)
        ...
        plt.tight_layout()
        print(f'save fig at ./figs/{dataset}_{method2name[method]}_valid_config.jpg')
        plt.savefig(f'./figs/{dataset}_{method2name[method]}_valid_config.jpg',dpi=800)