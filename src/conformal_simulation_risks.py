...
def main():
    ...
    evaluator: LLMSimulator = LLMSimulator(args=args, llm=llm)

    for task_name in args.llm_eval_tasks:
        logger.info('Conformal simulation risks on task: {}'.format(task_name))
        evaluator.calibrate_single_task(eval_dataset, task_name)

    # TODO: merge the function in conformal_compute_valid_configs here
    logger.info('Done')
